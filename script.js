...
function renderScoreHud() {
	if (isCasualGame()) {
		...
		scoreNode.innerText = `PUNKTY: ${state.game.score}`;
		scoreNode.style.display = 'block';
		cubeCountNode.style.opacity = 0.65 ;
	}
	cubeCountNode.innerText = `KOSTKI ZNISZCZONE: ${state.game.cubeCount}`;
}
...
function renderMenus() {
	...
	switch (state.menus.active) {
		...
		case MENU_SCORE:
			...
			if (isNewHighScore()) {
				highScoreLblNode.textContent = 'Nowy rekord!';
			} else {
				highScoreLblNode.textContent = `Rekord: ${formatNumber(getHighScore())}`;
			}
